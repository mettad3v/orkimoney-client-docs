...
<Info>
Use LIVE API keys with the production base URL and TEST API keys with the sandbox base URL. Sandbox lifecycle coverage varies by provider. Providers with a complete test lifecycle, including Stripe, can simulate checkout completion, provider callbacks, Orki transaction updates, and outgoing partner webhook delivery without moving real funds.
</Info>
